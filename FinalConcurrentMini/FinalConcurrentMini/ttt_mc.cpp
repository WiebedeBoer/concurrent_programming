...
int trials = 1000;
const int n_threads = 10; //threads
mutex slock; //score mutex lock
...
void mcUpdateScores(array<int, 9> &subscores, State &trialboard, Player &winner)
{
	...
	for (int i = 0; i < 9; i++)
	{
		slock.lock(); //lock score
		scores[i] = scores[i] + subscores[i];
		slock.unlock(); //unlock score
	}
}
...
void multiple(const State &board)
{
	for (int i = 0; i < trials; i++)
	{
		mcTrial(board); //for the trial board
	}
}
...
Move mcMove(State &board, const Player &player)
{
	...
	thread *th = new thread[n_threads - 1];


	for (int i = 0; i < n_threads - 1; ++i)
	{
		th[i] = thread(multiple, ref(board)); //make thread
	}

	multiple(board); //main thread

	for (int i = 0; i < n_threads - 1; ++i)
	{
		th[i].join(); //join the threads
	}
